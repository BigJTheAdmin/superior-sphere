...
type Service = {
  ...
  family: Family;
  method?: Method;
  notes?: string;        // human hint for “why this URL”
  requiresAuth?: boolean; // when true, we still list it but mark as unreachable unauth
};

// ===== 50 enterprise-common services (public, no-auth where possible) =====
const SERVICES: Service[] = [
  ...
  // Public metadata probe (NOT tenant health)
  ...
  // Security / SASE / Email Security / EDR (public status pages only)
  ...
  { name: "Okta Workforce Identity", family: "Security", url: "https://status.okta.com/api/v2/summary.json", method: "GET" }, // dup family ok
...
async function probe(url: string, method: Method = "GET", timeoutMs = TIMEOUT_MS) {
  ...
  try {
    ...
    // Fallback if some endpoints dislike HEAD (we default to GET anyway)
...
// simple p-map to cap concurrency
  ...
  async function worker() {
    while (true) {
      ...
      try {
        ...
        // never drop an item from output
...
export const GET: APIRoute = async () => {
  ...
  return new Response(JSON.stringify(body), {
    headers: {
      ...
      // allow browser to call this API route directly
