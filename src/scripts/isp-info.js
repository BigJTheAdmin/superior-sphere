...
// Full drop-in client script for the ISP Intelligence page (no Cloudflare Worker)
// All upstream calls go through same-origin /api/netfetch to avoid CORS.

import { safeFetchJSON } from "/src/lib/safeFetch.js";
import { enqueue } from "/src/lib/fetchQueue.js";

/* -----------------------------------------------------------
   Config: always proxy through the server route
   ----------------------------------------------------------- */
const PROXY = "/api/netfetch"; // same-origin server route
...
/* -----------------------------------------------------------
   Fetch helpers: always proxy via server
   ----------------------------------------------------------- */
...
const qJSON = (url) => enqueue(() => safeFetchJSON(proxify(url)));
